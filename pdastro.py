...
class pdastroclass:
    ...
    def fitsheader2table(self,fitsfilecolname,indices=None,requiredfitskeys=None,optionalfitskey=None,usegetheader=False,
                         raiseError=True,raiseError_badfitsfile=True,verify='silentfix',skipcolname=None,headercol=None,ext=None,extname=None,
                         prefix=None,suffix=None,null_value=None):
        ...
        for index in indices:
            try:
                ...
                if raiseError_badfitsfile:
                    raise
                else:
                    ixs_badfiles.append(index)
                    continue
                ...
                for fitskey in requiredfitskeys:
                    ...
                    if fitskey in header:
                        ...
                        if raiseError:
                            ...
                            self.t.loc[index,col]=null_value
                            if skipcolname is not None:
            ...
            if not usegetheader:
                hdu.close()
        if len(ixs_badfiles)>0:
            if requiredfitskeys!=None:
                for fitskey in requiredfitskeys:
                    col = fitskey2col(fitskey,prefix=prefix,suffix=suffix)
                    ixs_none = self.ix_is_null(col,ixs_badfiles)
                    if len(ixs_none)>0:
                        self.t.loc[ixs_none,col]=null_value
            if optionalfitskey!=None:
                for fitskey in optionalfitskey:
                    col = fitskey2col(fitskey,prefix=prefix,suffix=suffix)
                    ixs_none = self.ix_is_null(col,ixs_badfiles)
            ...
            if skipcolname is not None:
                self.t.loc[ixs_badfiles,skipcolname]=True
    ...
    def intializecols4statparams(self,params=None,prefix='',suffix='',parammapping={},skipparams=[],
        ...
        for param in params:
            ...
            if setcol2None or (not(outcol in self.t.columns)):
                if param in ['median','mean','mean_err','stdev','stdev_err','X2norm']:
